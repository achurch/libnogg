...
int main(void)
{
    ...
    uint8_t *data;
    long size;
    EXPECT_TRUE(f = fopen("tests/data/large-pages.ogg", "rb"));
    ...
    fclose(f);
    MODIFY(data[0x10A6AL], 0x45, 0x2A);
    MODIFY(data[0x10A6BL], 0xB4, 0xEA);
    MODIFY(data[0x10A6CL], 0x61, 0x02);
    MODIFY(data[0x10A6DL], 0xBA, 0x3E);
    MODIFY(data[0x10A71L], 0x01, 0x02);
    MODIFY(data[0x10A72L], 0x01, 0x00);

    vorbis_t *vorbis;
    EXPECT_TRUE(vorbis = vorbis_open_buffer(data, size, NULL));

    EXPECT_TRUE(vorbis_seek(vorbis, 882));
    float pcm[611];
