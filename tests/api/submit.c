...
int main(void)
{
    ...
    EXPECT_EQ(error, VORBIS_NO_ERROR);
    error = (vorbis_error_t)-1;
    /* Packets not embedded in Ogg streams can't be truncated, so we
     * receive the full frame size of 256 samples here even though the
     * original audio is only 40 samples long. */
    EXPECT_EQ(vorbis_read_float(vorbis, pcm+1, 256, &error), 255);
    EXPECT_EQ(error, VORBIS_ERROR_STREAM_END);
